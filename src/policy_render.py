...
def main(steps = None):
    ...
    data_folder = "checkpoints_RunningTest_DefaultXML"
    ...
    if steps is None:
        ...
        model = SAC.load(checkpoints[-1][1], device="mps") # this gets the latest checkpoint

    else:
        model = SAC.load(f"../checkpoints/{data_folder}/model_{steps}.zip", device="mps") # this gets the first checkpoint
...
if __name__ == "__main__":
    ...
    # pip show mujoco
    parser = argparse.ArgumentParser(description="MuJoCo Humanoidv5 policy renderer")
    parser.add_argument("--steps", type=int, default=250, help="Number of steps to run the policy")
